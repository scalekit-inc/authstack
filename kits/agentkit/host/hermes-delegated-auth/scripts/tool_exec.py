...
def _http_json(method: str, url: str, *, form: dict | None = None, headers: dict | None = None) -> dict:
...
def _multipart_file(filename: str, file_bytes: bytes, mime: str) -> tuple[bytes, str]:
...
# SCALEKIT_* matches hermes.mdx. TOOL_* is the OpenClaw alias.
TOOL_CLIENT_ID = _env('SCALEKIT_CLIENT_ID', 'TOOL_CLIENT_ID')
TOOL_CLIENT_SECRET = _env('SCALEKIT_CLIENT_SECRET', 'TOOL_CLIENT_SECRET')
TOOL_ENV_URL = _env('SCALEKIT_ENVIRONMENT_URL', 'TOOL_ENV_URL')
TOOL_IDENTIFIER = _env('SCALEKIT_IDENTIFIER', 'TOOL_IDENTIFIER')
...
def proxy_request(
    ...
    try:
        ...
        if input_file:
            ...
            form_data, content_type = _multipart_file(filename, file_bytes, file_mime)
...
def _get_connection_type(connection_name: str) -> str:
    ...
    while True:
        try:
            query = urlencode({"page_size": 30, "page_token": page_token})
            data = _http_json("GET", f"{url}?{query}", headers=headers)
        except Exception:
            return "OAUTH"
...
def list_connections(provider: str = None, page_size: int = 20) -> None:
    ...
    while True:
        try:
            query = urlencode({"page_size": page_size, "page_token": page_token})
            data = _http_json("GET", f"{url}?{query}", headers=headers)
        except Exception as e:
...
Required environment variables:
  ...
  SCALEKIT_ENVIRONMENT_URL / TOOL_ENV_URL
    ...
    parser.add_argument(
        ...
        help='Unique identifier for the connected account — required for --generate-link and --execute-tool. Defaults to SCALEKIT_IDENTIFIER or TOOL_IDENTIFIER.'
    )
    ...
    if not args.identifier:
        if not TOOL_IDENTIFIER:
            print(f"{RED}❌ Identifier is required. Set SCALEKIT_IDENTIFIER or pass --identifier.{RESET}")
            _exit(1)
        else:
