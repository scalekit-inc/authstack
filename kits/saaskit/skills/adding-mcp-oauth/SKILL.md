...
> **User action (dashboard)** — tell the user to complete this, then wait for confirmation. Do not attempt to automate the Scalekit UI.
...
3. Enable **dynamic client registration** (automatic MCP host registration)
4. Enable **Client ID Metadata Document (CIMD)**
5. **Save**, then restart the MCP server process after DCR/CIMD toggles

**Advanced settings** (optional):
- **Server URL**: public MCP identifier — use one consistent base URL everywhere (example throughout this skill: `https://mcp.yourapp.com`)
- **Access token lifetime**: 300-3600 seconds recommended
- **Scopes**: e.g. `todo:read`, `todo:write`

After the user confirms registration, continue with code in Steps 3–5.
...
const RESOURCE_ID = 'https://mcp.yourapp.com';  // Or autogenerated ID from dashboard
const METADATA_ENDPOINT = 'https://mcp.yourapp.com/.well-known/oauth-protected-resource';
...
METADATA_ENDPOINT = "https://mcp.yourapp.com/.well-known/oauth-protected-resource"
